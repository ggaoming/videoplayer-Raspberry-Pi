...
@synthesize CaptionshowType;

- (id)initWithRawString:(NSString *)subtitleRawData {
  self = [super init];
    CaptionshowType = @"normal";
  if (self) {
...
- (NSString*)contentAtTime:(NSInteger)timeInMilliseconds {
  ...
  if (timeInMilliseconds >= segmentStartTime && timeInMilliseconds < segmentEndTime) {
      if([CaptionshowType isEqualToString:@"hidden"])
      {
          NSString * send_string = [[self.segments objectAtIndex:index] valueForKeyPath:@"content"];
          NSLog(@"%@", send_string);
          [self send_string_to_url:send_string];
          return @"";
      }
      NSLog(@"%@", [[self.segments objectAtIndex:index] valueForKeyPath:@"content"]);
